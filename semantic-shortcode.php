...
final class SemanticShortcode {
	...
	private function init_hooks() {
		...
		add_filter( 'plugin_action_links_' . plugin_basename(__FILE__),  array($this,'ss_plugin_action_links') );
	}
    /**
     * Used to add link in plugin page
     */
	function ss_plugin_action_links( $links ) {
	   $links[] = '<a href="'. esc_url( get_admin_url(null, 'edit.php?page=semantic-shortcode') ) .'">Plugin Page</a>';
	   return $links;
	}
	/**
	...
	function register_semantic_shortcode_menu_page(){
		add_submenu_page( 'edit.php','Semantic Shortcode', 'Semantic Shortcode', 'manage_options', 'semantic-shortcode', 'fn_semantic_shortcode'); 
	}
	/**
	 * Define ss Constants
	 */
	...
	function ss_admin_style_script(){
		if(isset($_REQUEST['page'] ) && $_REQUEST['page'] == 'semantic-shortcode'):
			wp_enqueue_style('semantic',SS_CSS_URL."semantic.min.css");
			wp_enqueue_script('jquery-address',SS_JS_URL."jquery.address.min.js",array("jquery"));
			wp_enqueue_script('semantic',SS_JS_URL."semantic.min.js",array("jquery","jquery-address"));
		endif;
